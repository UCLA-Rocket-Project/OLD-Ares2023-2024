...
namespace IMU {
    ...
    void setupIMU() {
        for (int i=0; i<3000; i++) {
            if(HAL::imu.begin_SPI(HAL::IMU_CS, HAL::sensorSPI)) {
                DEBUGLN("found");
                break;
            }
        ...
        uint16_t accel_divisor = HAL::imu.getAccelRateDivisor();
        ...
        uint8_t gyro_divisor = HAL::imu.getGyroRateDivisor();
    ...
    void readIMU() {
        ...
        HAL::imu.getEvent(&accel, &gyro, &temp);
