...
void H3LIS::setI2CAddr(uint8_t address)
{
  this->address = address;
}

void H3LIS::setSPICSPin(uint8_t pin)
{
  this->CSPin = pin;
}

void H3LIS::axesEnable(bool enable)
{
  uint8_t data;
...
void H3LIS::setPowerMode(power_mode pmode)
{
  ...
  H3LIS_write(CTRL_REG1, &data, 1); // write the new value to CTRL_REG1
}

void H3LIS::setODR(data_rate drate)
{
  uint8_t data;
  H3LIS_read(CTRL_REG1, &data, 1);
...
void H3LIS::readAxes(int16_t &x, int16_t &y, int16_t &z)
{
  uint8_t data[6]; // create a buffer for our incoming data
  H3LIS_read(OUT_X_L, &data[0], 1);
  H3LIS_read(OUT_X_H, &data[1], 1);
  H3LIS_read(OUT_Y_L, &data[2], 1);
  H3LIS_read(OUT_Y_H, &data[3], 1);
  H3LIS_read(OUT_Z_L, &data[4], 1);
  H3LIS_read(OUT_Z_H, &data[5], 1);
...
uint8_t H3LIS::readReg(uint8_t reg_address)
{
  uint8_t data;
  H3LIS_read(reg_address, &data, 1);
  return data;
}

float H3LIS::convertToG(int maxScale, int reading)
{
  float result = (float(maxScale) * float(reading))/2047;
  return result;
}

void H3LIS::setHighPassCoeff(high_pass_cutoff_freq_cfg hpcoeff)
{
  ...
  uint8_t data;
  H3LIS_read(CTRL_REG2, &data, 1);
  data &= ~0xfc;  // Clear the two low bits of the CTRL_REG2
  data |= hpcoeff;
  H3LIS_write(CTRL_REG2, &data, 1);
}

void H3LIS::enableHPF(bool enable)
{
...
void H3LIS::HPFOnIntPin(bool enable, uint8_t pin)
{
...
void H3LIS::intActiveHigh(bool enable)
{
  // Are the int pins active high or active low?
  uint8_t data;
  H3LIS_read(CTRL_REG3, &data, 1);
  ...
  H3LIS_write(CTRL_REG3, &data, 1);
}

void H3LIS::intPinMode(pp_od _pinMode)
{
  uint8_t data;
  H3LIS_read(CTRL_REG3, &data, 1);
...
void H3LIS::latchInterrupt(bool enable, uint8_t intSource)
{
...
void H3LIS::intSrcConfig(int_sig_src src, uint8_t pin)
{
...
void H3LIS::setFullScale(fs_range range)
{
  uint8_t data; 
  H3LIS_read(CTRL_REG4, &data, 1);
  data &= ~0xcf;
  data |= range<<4;
  H3LIS_write(CTRL_REG4, &data, 1);
}

bool H3LIS::newXData()
{
  uint8_t data;
  H3LIS_read(STATUS_REG, &data, 1);
  if (data & 1<<0)
  {
    return true;
  }
...
void H3LIS::enableInterrupt(int_axis axis, trig_on_level trigLevel,
                     uint8_t interrupt, bool enable)
{
  ...
  H3LIS_write(reg, &data, 1);
}

void H3LIS::setIntDuration(uint8_t duration, uint8_t intSource)
{
  if (intSource == 1)
  {
    H3LIS_write(INT1_DURATION, &duration, 1);
  }
  else
  {
    H3LIS_write(INT2_DURATION, &duration, 1);
  }
}

void H3LIS::setIntThreshold(uint8_t threshold, uint8_t intSource)
{
  if (intSource == 1)
  {
    H3LIS_write(INT1_THS, &threshold, 1);
  }
  else
  {
    H3LIS_write(INT2_THS, &threshold, 1);
  }
}

void H3LIS::H3LIS_write(uint8_t reg_address, uint8_t *data, uint8_t len)
{
  if (mode == USE_I2C)
  {
  ...
  else
  {
    ...
    H3LIS_SPI->transfer(reg_address | 0x40);
    for (int i=0; i<len; i++)
    {
      H3LIS_SPI->transfer(data[i]);
    }
    digitalWrite(CSPin, HIGH);
  }
}

void H3LIS::H3LIS_read(uint8_t reg_address, uint8_t *data, uint8_t len)
{
  if (mode == USE_I2C)
  {
  ...
  else
  {
    ...
    digitalWrite(CSPin, LOW);
    H3LIS_SPI->transfer(reg_address | 0xC0);
    for (int i=0; i<len; i++)
    {
      data[i] = H3LIS_SPI->transfer(0);
    }
    digitalWrite(CSPin, HIGH);
  }
}
