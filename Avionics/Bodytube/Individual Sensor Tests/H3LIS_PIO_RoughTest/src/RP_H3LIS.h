...
typedef enum {PUSH_PULL, O_DRAIN} pp_od;
...
public:
  ...
  // public functions
  H3LIS();   // Constructor. Defers all functionality to .begin()
  void begin(comm_mode mode, uint8_t CS_pin, SPIClass* SPI_port);
  ...
  uint8_t CSPin;
  void H3LIS_write(uint8_t address, uint8_t *data, uint8_t len);
  void H3LIS_read(uint8_t address, uint8_t *data, uint8_t len);
};
